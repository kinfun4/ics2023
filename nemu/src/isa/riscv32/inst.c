...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + imm, R(rd) = s->pc + 4, func_call(s->pc, s->dnpc));
  ...
  INSTPAT("0000000 00000 00001 000 00000 11001 11", ret    , I, s->dnpc = src1 - (src1 & 1), func_ret(s->pc,s->dnpc));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src1 + imm) - ((src1 + imm) & 1), R(rd) = s->pc + 4, func_call(s->pc, s->dnpc));
