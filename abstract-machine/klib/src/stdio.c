...
do {\
  if(state==0){assert(str!=NULL);*(str++) = c;}\
  else putch(c);\
} while (0)
...
do {\
  ...
  if(space_or_zero){\
    while(buf_pt - buf < space) WRITE(buf_pt,'0',0);\
  }\
  if(is_negtive) WRITE(buf_pt, '-',0);\
  else if(is_show_sign) WRITE(buf_pt, '+',0);\
  int siz = buf_pt-buf;\
  if(right_or_left){\
    while(buf_pt > buf){WRITE(out, *(buf_pt-1),state);buf_pt--;}\
    while(siz < space) {WRITE(out,' ',state);space--;}\
  }\
  else{\
    while(siz < space){WRITE(out,' ',state);space--;}\
    while(buf_pt > buf){WRITE(out, *(buf_pt-1),state);buf_pt--;}\
...
do {\
  ...
  if(is_formed && base == 8) WRITE(buf_pt, '0',0);\
  if(is_formed && base == 16) {WRITE(buf_pt, 'x',0);WRITE(buf_pt, '0',0);}\
  int siz = buf_pt-buf;\
  if(right_or_left){\
    while(buf_pt > buf){WRITE(out, *(buf_pt-1),state);buf_pt--;}\
    while(siz < space) {WRITE(out,' ',state);space--;}\
...
do {\
 switch (*fmt) {\
  case 'c':PROCESS_CHAR(out,ap,state);fmt++;break;\
  case 's':PROCESS_STRING(out,ap,state);fmt++;break;\
  case 'd':PROCESS_SIGNED(out,ap,state);fmt++;break;\
  case 'i':PROCESS_SIGNED(out,ap,state);fmt++;break;\
  case 'u':PROCESS_UNSIGNED(out,ap,state);fmt++;break;\
  case 'o':base=8;PROCESS_UNSIGNED(out,ap,state);fmt++;break;\
  case 'x':base=16;PROCESS_UNSIGNED(out,ap,state);fmt++;break;\
  case 'X':base=16;is_capital=1;PROCESS_UNSIGNED(out,ap,state);fmt++;break;\
  case 'p':is_formed=1;base=16;PROCESS_UNSIGNED(out,ap,state);fmt++;break;\
  case '%':WRITE(out, '%',state);fmt++;break;\
  default: assert(0);\
 }\
} while (0)
